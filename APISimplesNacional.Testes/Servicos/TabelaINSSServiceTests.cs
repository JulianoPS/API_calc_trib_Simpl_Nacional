...
{
    public class TabelaINSSServiceTests
    {
        private readonly Mock<ITabelaINSSRepositorio> _repoMock;
        private readonly TabelaINSSService _service;

        public TabelaINSSServiceTests()
        {
            _repoMock = new Mock<ITabelaINSSRepositorio>();
            var empMock = Mocks.EmpresaServiceMock.Get();
            _service = new TabelaINSSService(empMock.Object, _repoMock.Object);
        }
        ...
        public async Task ObterPorEmailOuCelularAsync_ShouldReturnDefault_WhenEmpresaNotFound()
        {
            ...
            var serv = new TabelaINSSService(empNull.Object, _repoMock.Object);
            var defaultEnt = new List<TabelaINSS> { new TabelaINSS { IdEmpresa = 1, Faixa = 1 } };
        ...
        public async Task AtualizarAsync_ShouldThrow_WhenEmpresaDefault()
        {
            ...
            var serv = new TabelaINSSService(empDefault.Object, _repoMock.Object);
            await Assert.ThrowsAsync<InvalidOperationException>(
                () => serv.AtualizarAsync("e", "c", new List<TabelaINSSDto>())
            );
        }
        ...
        {
            var lista = new List<TabelaINSSDto> { new TabelaINSSDto { Faixa = 2, LimiteInic = 0, LimiteFin = 100, Aliquota = 0.1m } };
            await _service.AtualizarAsync("e", "c", lista);
            _repoMock.Verify(
                r => r.AtualizarAsync(
                    It.Is<List<TabelaINSS>>(l => l.Count == 1 && l[0].Faixa == 2)
                ),
                Times.Once
            );
        }
